...
describe("通知设置：设置态 ↔ 表单态", () => {
  it("toForm 回显总开关、撤食开关与提前天数文本（作废的分类子开关不进表单）", () => {
  ...
  it("toSettings 带上总开关、撤食开关与提前天数，作废分类开关固定回写 true，autostart 原样透传", () => {
    const out = toSettings({ master: false, retrieval: false, daysAheadText: "3" }, true);
    expect(out).toEqual({
...
describe("通知设置：提前天数校验", () => {
  ...
  it("非法提前天数时 toSettings 返回 null（不落库）", () => {
    const out = toSettings(
      { master: true, retrieval: true, daysAheadText: "-1" },
      true,
    );
    expect(out).toBeNull();
